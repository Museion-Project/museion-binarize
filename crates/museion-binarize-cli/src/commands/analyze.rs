...
use museion_binarize_core::analysis::{self, DocumentAnalysisReport};
use museion_binarize_core::error::CoreError;
use museion_binarize_core::pdfium_backend::PdfiumConfig;
use museion_binarize_core::pipeline::{self, AnalysisOptions};
...
fn print_human(report: &DocumentAnalysisReport) {
    ...
    for page in &report.pages {
        let threshold_display = match &page.threshold {
            ...
            museion_binarize_core::image_pipeline::ThresholdInfo::Manual { threshold } => {
                threshold.to_string()
            }
            museion_binarize_core::image_pipeline::ThresholdInfo::Sauvola { .. } => {
                "adaptive".to_string()
            }
        };
        println!(
