//! Versioned benchmark report types: `mpdf-benchmark`.
//!
...
pub const BENCHMARK_REPORT_SCHEMA: &str = "mpdf-benchmark";
...
pub enum BenchmarkLevel {
    /// `degraded raster -> M PDF image pipeline -> binary output`.
    /// The primary fidelity benchmark; does not involve PDFium.
    Raster,
    /// `PDF -> PDFium render -> M PDF image pipeline -> binary output`.
...
mod tests {
    ...
    fn benchmark_report_schema_constants_are_1_0() {
        assert_eq!(BENCHMARK_REPORT_SCHEMA, "mpdf-benchmark");
